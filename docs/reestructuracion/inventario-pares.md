...
| E7 | **`CLAUDE.md` bajo 200 líneas — pero lo que se vigila es el total combinado** — `plan.md` cita la recomendación · retrospectiva §2.8 midió 213 · **233 el 14/8/2026** (`wc -l CLAUDE.md`) | (A) 🔒 — el número sube solo y no hay mecanismo de contención. El "hoy 217" que decía esta fila era del 10/8 y quedó viejo sin que nada avisara, que es el par mordiéndose la cola. **`CLAUDE.md` solo no es la métrica:** las instrucciones permanentes del chat de criterio son la otra mitad del mismo presupuesto de contexto, crecen aparte y nadie las suma. **Baseline 674 = 441 (chat) + 233 (`CLAUDE.md`); umbral 750, o +15% en una ronda.** El lado chat **no se mide desde el repo** —lo pasa Joaco, 441 el 15/8/2026— y solo se puede sumar si se cuenta igual que el otro lado: **líneas, no palabras ni tokens** |
