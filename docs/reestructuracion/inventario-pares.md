...
| E7 | **`CLAUDE.md` bajo 200 líneas — pero lo que se vigila es el total combinado** — `plan.md` cita la recomendación · retrospectiva §2.8 midió 213 · **243 el 16/8/2026** (`wc -l CLAUDE.md`) | (A) 🔒 — el número sube solo y no hay mecanismo de contención. El "hoy 217" que decía esta fila era del 10/8 y quedó viejo sin que nada avisara, que es el par mordiéndose la cola. **`CLAUDE.md` solo no es la métrica:** las instrucciones permanentes del chat de criterio son la otra mitad del mismo presupuesto de contexto, crecen aparte y nadie las suma. **Total 940 = 468 (chat) + 472 (repo) el 16/8/2026, contra un umbral de 750: pasado por 190 líneas.** El lado repo se mide con `cat CLAUDE.md .claude/rules/commits.md .claude/rules/limites.md .claude/rules/herramientas.md ~/.claude/CLAUDE.md | wc -l` — las tres rules sin `paths:` son residentes igual que `CLAUDE.md`, y `gdscript.md` y `netcode.md` no entran porque cargan por archivo tocado. El lado chat **no se mide desde el repo** —lo pasa Joaco, 468 el 16/8/2026— y solo se puede sumar si se cuenta igual que el otro lado: **líneas, no palabras ni tokens**. **Casi todo el salto contra el baseline viejo (674 = 441 + 233) es remedición y no crecimiento:** 441→468 del lado chat es volver a contar, y 243→472 del lado repo es haber incluido por fin las tres rules residentes y el global, que siempre estuvieron cargándose. Lo único que creció de verdad es 233→243, las 10 líneas que `CLAUDE.md` sumó el 16/8/2026. **Si el umbral estaba mal fijado o si hay que recortar de verdad no está decidido, y se decide aparte** |
